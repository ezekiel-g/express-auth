...
beforeEach(() => {
  sessionController.readSession.mockImplementation((req, res) => {
    res.status(200).json([{ id: 1, username: 'User1' }]);
  });
  ...
  sessionController.refreshSession.mockImplementation((req, res) => {
    res.status(200).json({ message: 'Session refreshed' });
  });

  sessionController.verifyTotp.mockImplementation((req, res) => {
...
describe('sessionRoutes', () => {
  ...
  it('handles POST /api/v1/sessions/refresh-session', async () => {
    const response = await request(app).post(
      '/api/v1/sessions/refresh-session',
    );
    expect(response.status).toBe(200);
  });

  it('handles POST /api/v1/sessions/verify-totp', async () => {
    const response = await request(app).post('/api/v1/sessions/verify-totp');
    expect(response.status).toBe(200);
  });
