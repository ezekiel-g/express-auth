...
beforeEach(() => {
  verificationController.verifyAccountByEmail.mockImplementation(
    (req, res) => {
      res.status(200).json({
  ...
  verificationController.confirmEmailChange.mockImplementation(
    (req, res) => {
      res.status(200).json({
  ...
  verificationController.getTotpSecret.mockImplementation(
    (req, res) => {
      res.status(200).json({
  ...
  verificationController.resendVerificationEmail.mockImplementation(
    (req, res) => {
      res.status(200).json({
  ...
  verificationController.sendPasswordResetEmail.mockImplementation(
    (req, res) => {
      res.status(200).json({
  ...
  verificationController.requestDeleteUser.mockImplementation(
    (req, res) => {
      res.status(200).json({
  ...
  verificationController.setTotpAuth.mockImplementation((req, res) => {
    res.status(200).json({
      message: 'Two-factor authentication enabled successfully',
    });
  });

  verificationController.resetPassword.mockImplementation(
    (req, res) => {
      res.status(200).json({
...
describe('verificationRoutes', () => {
  ...
  it('handles GET /api/v1/verifications/verify-account-by-email', async () => {
  ...
  it('handles POST /api/v1/verifications/' +
  ...
  it('handles POST /api/v1/verifications' +
      '/request-account-deletion', async () => {
    const response = await request(app).post(
