...
const createSession = async (request, response) => {
  ...
  try {
    const accessToken = jsonwebtoken.sign({ id: user.id }, jwtSecret, {
      expiresIn: '1h',
    });

    const refreshToken = jsonwebtoken.sign({ id: user.id }, jwtRefreshSecret, {
      expiresIn: '7d',
    });
...
const refreshSession = async (request, response) => {
  ...
  try {
    const decryptedToken = jsonwebtoken.verify(refreshToken, jwtRefreshSecret);
